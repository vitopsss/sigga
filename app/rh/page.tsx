...
function getVinculoBadge(vinculo: string | null) {
  ...
  if (normalized.includes("clt") || normalized.includes("contratado")) return <Badge variant="success">CLT</Badge>;
  if (normalized.includes("bolsa") || normalized.includes("bolsista")) return <Badge variant="info">Bolsista</Badge>;
  if (normalized.includes("pj")) return <Badge variant="warning">PJ</Badge>;
...
export default async function RHPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  let colaboradores: ColaboradorListItem[] = [];
  ...
  try {
    colaboradores = await prisma.colaborador.findMany({
      where: buscaNormalizada
        ? {
            OR: [
              { idRH: { contains: buscaNormalizada, mode: "insensitive" } },
              { cargo: { contains: buscaNormalizada, mode: "insensitive" } },
              { vinculo: { contains: buscaNormalizada, mode: "insensitive" } },
            ],
          }
        : undefined,
      include: { cadastro: true },
  ...
  const totalSalarios = colaboradores.reduce((t, c) => t + Number(c.salarioBase), 0);
        ...
        <Card>
          ...
          {colaboradores.length === 0 ? (
            <Empty>
              ...
              <EmptyActions>
                <Link href="/rh/novo"><Button variant="primary">Novo Colaborador</Button></Link>
              </EmptyActions>
            </Empty>
          ) : (
            <Table>
              ...
              <TableBody>
                {colaboradores.map((colab) => (
                  <TableRow key={colab.id}>
                    ...
                    <TableCell className="font-semibold text-zinc-950">{currencyFormatter.format(Number(colab.salarioBase))}</TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Link href={`/cadastros/${colab.cadastro.id}`}>
                          <Button variant="ghost" size="icon"><Building2 className="h-4 w-4" /></Button>
                        </Link>
